...
```{r echo=FALSE}
mpgVsTrans <- data.frame(mpg=mtcars$mpg,trans=as.factor(mtcars$am))
levels(mpgVsTrans$trans) <- c('Automatic', 'Manual')

automatic.mean <- round(mean(mpgVsTrans[mpgVsTrans$trans == 'Automatic', 1]),2)
manual.mean <- round(mean(mpgVsTrans[mpgVsTrans$trans == 'Manual', 1]),2)
